...
def prepare_context_for_llm(query: str, search_results: List[Dict]) -> str:
    ...
    for i, result in enumerate(search_results[:3], 1):
        food_context = []
        food_context.append(f"Option {ii: {result['food_name']}}")
...
def generate_llm_rag_response(query: str, search_results: List[Dict]) -> str:
    ...
    try:
        ...
        1. Aknowledges the user's request
...
def generate_fallback_response(query: str, search_results: List[Dict]) -> str:
    ...
    if not search_results:
        return "I couldn't find any food items matching your request. Try describing what your're in the mood for with different words!"
